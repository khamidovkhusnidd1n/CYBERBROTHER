...
# Default 50 Slots for Slot-system
DEFAULT_SLOTS = [
...
# --- LICENSE LOGIC (STEALTH V2) ---
def is_valid_stealth_id(device_id):
    """Hidden logic to verify if the ID was generated by our authentic algorithm."""
    try:
        if not device_id.startswith("CYB-") or len(device_id) != 16:
            return False
            
        alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"
        body = device_id[4:8] + device_id[9:13]
        checksum = device_id[14:16]
        
        # Re-calculate hidden checksum
        ascii_sum = sum(ord(c) for c in body)
        expected_c1 = alphabet[(ascii_sum * 7) % len(alphabet)]
        expected_c2 = alphabet[(ascii_sum + 31) % len(alphabet)]
        
        return checksum == f"{expected_c1}{expected_c2}"
    except:
        return False

def generate_license_key(device_id):
    """Generates the Pro key using the same crypto salt as the app."""
    if not is_valid_stealth_id(device_id):
    ...
    # Matching the app's hash segment: .substring(5, 21)
    full_hash = hashlib.sha256(raw.encode()).hexdigest()
    return full_hash[5:21].upper()
...
async def handle_text_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == "🛒 Premium Olish":
        ...
        # 🛡️ STEALTH V2 VALIDATION 🛡️
        if not is_valid_stealth_id(device_id):
            await update.message.reply_text(
                "❌ *Xatolik: Noto'g'ri Device ID!*\n\n"
                "Iltimos, ilovadagi Settings -> Subscription bo'limidan nusxa oling.",
                parse_mode='Markdown'
            )
            return
        ...
        # Generate the unique license key using the stealth algorithm
        license_key = generate_license_key(device_id)

        if device_id in verified_devices and verified_devices[device_id]['status'] == 'active':
             await update.message.reply_text(
                f"✅ *Sizning kalitingiz:* \n\n`{license_key}`",
                parse_mode='Markdown',
                reply_markup=get_main_keyboard(is_admin)
            )
             context.user_data.clear()
        ...
        # Auto-register new verified device
        verified_devices[device_id] = {
            'status': 'active',
            'assigned_to': user.id,
            'key': license_key,
        ...
        context.user_data.clear()
        await update.message.reply_text(
            f"✅ *Tabriklaymiz!*\nLitsenziya muvaffaqiyatli faollashtirildi.\n\n🔑 Kalitingiz: `{license_key}`\n\nIlovaga nusxalab kiriting.", 
            parse_mode='Markdown', 
            reply_markup=get_main_keyboard(is_admin)
        )
